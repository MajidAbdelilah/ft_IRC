...
void Server::ReceiveNewData(int fd)
{
	...
	{ //-> print the received data
		...
		client.addCommand(client.parseIrcResponse(data));
...
void Client::printCommands()
{
	...
	{
		std::cout << "Command: " << commands[i].command << std::endl;
		std::cout << "Prefix: " << commands[i].prefix << std::endl;
		size_t j = 0;
		while(j < commands[i].params.size())
		{
			std::cout << "Param: " << commands[i].params[j] << std::endl;
			j++;
		}
		i++;
	}
...
IrcCommand Client::parseIrcResponse(std::string cmd)
{
	std::string prefix = "";
	std::string command = "";
	...
	if (temp != "")
		params.push_back(temp);
	return IrcCommand(command, prefix, params);
}
